...
void PlayerBehaviourScript::setAnimationActive(std::string aAnimationTag,
                                               bool aState) {
  for (auto animation : mGameObject->getComponents<Animation>()) {
...
void PlayerBehaviourScript::handleAnimations() {
  if (mGameObject->hasComponent<NetworkObject>()) {
    NetworkObject *networkObject =
        mGameObject->getComponents<NetworkObject>()[0];
    if (!networkObject) {
      return;
    }
  ...
  if (abs(previousTransform.position.x - currentTransform.position.x) < 0.1 &&
    ...
    if ((currentActiveAnimationTag() == "playerIdleFront") ||
        (currentActiveAnimationTag() == "playerIdleBack") ||
    ...
    if (currentTransform.position.y < previousTransform.position.y) {
      if (abs(currentTransform.position.x - previousTransform.position.x) >
          0.1) {
        deactivateAllAnimations();
        setAnimationActive("playerWalkingBackSide", true);
      } else if (abs(currentTransform.position.x -
                     previousTransform.position.x) < 0.1) {
        deactivateAllAnimations();
...
void PlayerBehaviourScript::hanldeCameraMovement() {
  ...
  Camera &currentCam = EngineBravo::getInstance()
                           .getSceneManager()
                           .getCurrentScene()
                           ->getActiveCamera();
