...
void RoomBehaviourScript::onStart() {
  ...
  std::string doorSpriteSheetPath =
      fsConverter.getResourcePath("Dungeontileset/atlas_walls_high-16x32.png");
  std::string enemySpritePath = fsConverter.getResourcePath(
...
void RoomBehaviourScript::updateDoors(const SpriteDef &spriteDef) {
  EngineBravo &engine = EngineBravo::getInstance();
  SceneManager &sceneManager = engine.getSceneManager();
  std::vector<GameObject *> doorGameObjects =
  ...
  for (size_t i = 0; i < doorGameObjects.size(); ++i) {
    ...
    std::vector<BoxCollider *> colliders =
        doorPart->getComponents<BoxCollider>();
    if (!colliders.empty()) {
